...
class SupernodeTransformer:
    ...
    def extract_supernodes(
        ...
        self.logger.info(f"Contracción Topológica Finalizada: {num_classes} clases reducidas a {num_supernodes} supernodos.")
...
class PILSGeometryManager:
    ...
    def validate_rational_outline_existence(self) -> bool:
        ...
        if sum_partitions > self.order:
            self.logger.warning(f"Infactibilidad geométrica detectada (Suma={sum_partitions} > Orden={self.order}).")
            return False

        for idx, sub_size in enumerate(self.partitions):
            if sub_size > self.order // 2:
                self.logger.warning(f"Advertencia teórica (Teorema Cruse): Supernodo {idx} (tamaño {sub_size}) domina el tablero.")
                return False
        
        self.logger.info("Estructura PILS validada exitosamente mediante contornos racionales simétricos.")
        return True
    ...
    def generate_latin_board_constellation(self) -> LatinBoard:
        board = LatinBoard(size=self.order)
        pils_mask = self.build_pils_orthogonal_mask()
        for r in range(self.order):
            board.add_asterism(f"row_asterism_{r}", {(r, c) for c in range(self.order) if pils_mask[r, c]})
        for c in range(self.order):
            board.add_asterism(f"col_asterism_{c}", {(r, c) for r in range(self.order) if pils_mask[r, c]})
        return board
...
class StochasticPackingRounder:
    ...
    def _build_lp_relaxation(
        ...
        super_weights: Dict[int, int]  # <--- NUEVO PARÁMETRO VITAL
    ):
        num_vars = num_supernodes * self.num_slots
        c = np.full(num_vars, -1.0) 
        A_eq, b_eq, A_ub, b_ub = [], [], [], []
        
        # 1. Asignación exacta proporcional al tamaño del Supernodo (Multicoloreo)
        for i in range(num_supernodes):
            row = np.zeros(num_vars)
            for j in range(self.num_slots): row[i * self.num_slots + j] = 1.0
            A_eq.append(row)
            b_eq.append(float(super_weights[i])) # <--- CORRECCIÓN MATEMÁTICA: Demanda W
            
        # ... (resto de las restricciones de clique y pairwise igual)
            
        # 2. INYECCIÓN ESTRICTA DE FACETAS DE CLIQUE (LP Hardening)
        # Convierte los SameAttendees/NotOverlap directamente en restricciones sum <= 1
        cliques_added = 0
        for clique in strict_cliques:
            ...
            for j in range(self.num_slots):
                row = np.zeros(num_vars)
                for i in clique:
                    row[i * self.num_slots + j] = 1.0
                A_ub.append(row)
                b_ub.append(1.0)
            cliques_added += 1
            
        self.logger.info(f"  Inyectados {cliques_added} Hiperplanos de Clique estricto en el modelo LP.")

        # 3. Restricciones Pairwise (solo de soporte para lo que no cubran los cliques explícitos)
        for i in range(num_supernodes):
            for k in range(i + 1, num_supernodes):
                if super_conflict_matrix[i, k] == 1:
                    for j in range(self.num_slots):
                        row = np.zeros(num_vars)
                        row[i * self.num_slots + j] = 1.0
                        row[k * self.num_slots + j] = 1.0
                        A_ub.append(row)
                        b_ub.append(1.0)

        # 4. Límites (Bounds) y PILS Domains
    ...
    def solve_fractional_landscape(
        ...
        c, A_eq, b_eq, A_ub, b_ub, bounds = self._build_lp_relaxation(
            num_supernodes, super_conflict_matrix, super_allowed_slots, strict_cliques
        )
        
        self.logger.info("Resolviendo LP continuo (Método HiGHS)...")
...
class CSPSolver:
    def __init__(self, inst: Instance, oracle: GNNOracle, W: np.ndarray, idx: Dict[str, int], cfg):
        ...
        # 1. Extraer Supernodos (Para el LP)
        from collections import defaultdict # Asegúrate de tener esto en tus imports iniciales
        transformer = SupernodeTransformer(self.logger)
        supernodes, super_W, super_weights, super_allowed = transformer.extract_supernodes(num_classes, conflict_matrix, allowed_slots)
        
        self.logger.info(f"  [Diagnóstico] {num_classes} clases mapeadas en {len(supernodes)} Supernodos.")
        for sid, w in super_weights.items():
            if w > 1:
                self.logger.debug(f"    Supernodo {sid} detectado como CLIQUE de tamaño {w}.")

        # 1.5 EXTRAER CLIQUES ESTRICTOS (Para inyectar cortes duros en el LP)
        class_id_to_sid = {}
        ...
        # Cliques por Distribuciones (SameAttendees / NotOverlap / SameTime)
        for dist in self.inst.distributions:
            if dist.required and dist.dtype in ("SameAttendees", "NotOverlap", "DifferentTime", "SameTime"):
                if len(dist.classes) > 1:
                    s_clique = frozenset({class_id_to_sid[c] for c in dist.classes if c in class_id_to_sid})
                    if len(s_clique) > 1: strict_cliques.add(s_clique)

        # 2. Relajación LP (Packing con pesos de demanda reales)
        self.fractional_guide = {}
        if SCIPY_AVAILABLE and linprog is not None:
            ...
            # 3. EXPANSIÓN FRACCIONAL: Proyectar probabilidades a cada variable INDIVIDUAL
            for s_id, c_indices in supernodes.items():
                for c_idx in c_indices:
                    cid = class_ids[c_idx]
                    self.fractional_guide[cid] = x_star[s_id]
                    
        # 4. Inicializar dominios sobre las variables INDIVIDUALES para el CSP
        self.unassigned_vars = list(self.inst.classes.keys())
        self.domains = self._build_individual_domains() 
        ...
        # Buscar "Salas Escasas" usando self.domains (Corregido)
        self._scarce_rooms: Set[str] = set()
        for cid, dom in self.domains.items():
    ...
    # ==========================================================
    # NUEVA FUNCIÓN: Agrega esta función justo debajo del __init__
    # ==========================================================
    def _build_individual_domains(self) -> Dict[str, List[Tuple[str, str]]]:
        """
        Construye los dominios válidos (Tiempo x Sala) cruzados con la disponibilidad
        para cada clase INDIVIDUAL, devolviendo al CSP su naturaleza granular pura.
        """
        domains = {}
        for cid in self.unassigned_vars:
            cls = self.inst.classes[cid]
            
            allowed_times = set(cls.allowed_times.keys()) if cls.allowed_times else set(self.ts_list)
            
            if not cls.room_required:
                allowed_rooms = {"NO_ROOM"}
            else:
                allowed_rooms = set(cls.allowed_rooms.keys()) if cls.allowed_rooms else set(self.inst.rooms.keys())
                
            # Filtrar pares donde la sala esté indisponible estructuralmente
            valid_pairs = [(t, r) for t in allowed_times for r in allowed_rooms if self._room_available_for_ts(r, t, cls.length)]
            domains[cid] = valid_pairs
    ...
    def _order_values(self, var: str, domain: List[Tuple[str, str]]) -> List[Tuple[str, str]]:
        ...
        # Debug Crítico
        if len(self.bt_nodes) % 5000 == 0:
             self.logger.debug(f"  [Diagnóstico CSP] Eval. {self.bt_nodes} | Var: {var} | Dominio residual: {len(domain)}")
        ...
        def score(val):
            ...
            if var in self.fractional_guide:
                try:
                    ...
                    prob_bonus = -2000.0 * prob  # Fuerte peso al prior de la relajación
                except ValueError:
                    pass

            return t_pen + r_pen + prob_bonus + random.uniform(0, 0.5)
        ...
        def score(val):
            ...
            prob_bonus = 0.0
            if total_prob > 0 and val in prob_map:
        ...
        return sorted(valid_domain, key=score)
    ...
    def _greedy_with_restarts(self, unassigned_meta: List[str], meta_domains: Dict[str, List]) -> Dict[str, Assignment]:
        ...
        for restart in range(max_restarts):
            ...
            nogoods: Dict[str, Set[Tuple[str, str]]] = {v: set() for v in unassigned_meta}
            ...
            while pending:
                ...
                # 1. Limpieza y validación de Dominio ANTES de puntuar
                valid_domain = []
                culprits_for_val = {}
                
                for ts, room in meta_domains.get(var, []):
                    if (ts, room) in nogoods[var]: 
                        continue
                    self.bt_nodes += 1
                    culprits = self._meta_conflicts_with_culprits(var, ts, room, current_meta_assignment)
                    if not culprits:
                        valid_domain.append((ts, room))
                    else:
                        culprits_for_val[(ts, room)] = culprits

                # 2. Redondeo Estocástico Dependiente (Solo sobre las opciones sobrevivientes)
                placed = False
                if valid_domain:
                    ordered_domain = self._order_values(var, valid_domain)
                    best_ts, best_room = ordered_domain[0]
                    current_meta_assignment[var] = Assignment(timeslot=best_ts, room=best_room)
                    assignment_order.append(var)
                    placed = True
                    stalled.clear()
                    continue

                # 3. Domain Wipe-out: Backjumping y acumulación de culpables reales
                backtracks_this_restart += 1
                
                all_culprits = set()
                for c in culprits_for_val.values():
                    all_culprits |= c

                if not all_culprits or not assignment_order:
                    if var in stalled:
                        continue
                    stalled.add(var)
                    pending.append(var)
    ...
    def _try_repair_insert(self, v: str, assignment: Dict[str, Assignment], meta_domains: Dict[str, List], max_moves: int = 4) -> bool:
        # Pre-filtrar opciones limpias para optimizar reparaciones locales
        valid_domain = []
        for ts, room in meta_domains.get(v, []):
            if not self._meta_conflicts(v, ts, room, assignment):
                valid_domain.append((ts, room))
        
        # En esta fase fallback, si el dominio libre es vacío, intentamos reubicar a la fuerza bruta
        if not valid_domain:
            valid_domain = meta_domains.get(v, [])
            
        for ts, room in self._order_values(v, valid_domain):
